...
### Fixed
- **„In der Nähe"-Liste im Detail-Panel war vom selben Cloud-IP-Block betroffen wie der Haupt-POI-Fetch.** `/api/nearby` lief noch über den Supabase-Proxy, der von den Overpass-Mirrors weiterhin abgelehnt wird. Klassifizierung (`classifyKind`), Distanzberechnung (`haversineMeters`) und Query-Bau wurden nach `src/client/features/poi-detail/nearbyClient.ts` portiert; `loadNearby` ruft jetzt direkt die Mirrors auf (geteilte Fallback-Logik mit dem Haupt-POI-Fetch aus `core/overpassFetch.ts`, wohin die Mirror-Fallback-Schleife aus `OverpassClient.ts` extrahiert wurde). Live im Browser getestet: 15 klassifizierte Nahversorger (Supermarkt, Apotheke, Bäckerei) korrekt nach Entfernung sortiert. `/api/nearby` bleibt als Endpoint bestehen, wird vom Client aber nicht mehr aufgerufen.
